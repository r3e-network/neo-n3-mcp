...
The server registers 48 public MCP tools spanning Neo N3 and Neo X, three fixed network resources, and one parameterized block resource. Tools that both chains implement require an explicit `chain` argument with no default. Tool registration, chain routing, and generic contract-reference handling are tested.
