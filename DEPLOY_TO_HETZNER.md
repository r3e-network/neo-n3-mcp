...
| 代码目录 | `/opt/neo-n3-mcp` |
| systemd 单元 | `/etc/systemd/system/neo-n3-mcp.service` |
| 环境变量文件 | `/etc/neo-n3-mcp/env`（`600 root:root`） |
...
sudo systemctl restart neo-n3-mcp
journalctl -u neo-n3-mcp -n 50 --no-pager
```
...
systemctl is-active neo-n3-mcp
systemctl is-enabled neo-n3-mcp
ss -tlnp | grep 3001
...
| 现象 | 检查 |
| --- | --- |
| 助手 503 | `systemctl status neo-n3-mcp`、`journalctl -u neo-n3-mcp -n 100` |
| 助手 401 | Vercel `NEOX_MCP_BEARER` 与 `/etc/neo-n3-mcp/env` 是否同值 |
| 助手报明文传输被拒 | `NEOX_MCP_URL` 是否仍是 `http://`，改为 `https://mcp.n3index.dev/mcp` 后重新部署 |
| 重启后服务消失 | `systemctl is-enabled neo-n3-mcp` 应为 `enabled`；若为 `static`/`disabled`，重跑第 1 节 |
